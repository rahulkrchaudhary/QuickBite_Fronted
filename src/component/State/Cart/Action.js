...
export const getAllCartItems=(reData)=>{
    return async (dispatch)=>{
        dispatch({type: GET_ALL_CART_ITEMS_REQUEST})
        try{
            const response = await api.get(`/api/carts/${reData.cartId}/items`, {
                headers:{
...
export const updateCartItem=(reData)=>{
    return async (dispatch)=>{
        dispatch({type: UPDATE_CARTITEM_REQUEST})
        try{
            const {data} = await api.put(`/api/cart-item/update`, reData.data, {
                headers:{
                    Authorization: `Bearer ${reData.jwt}`
                }
            })
